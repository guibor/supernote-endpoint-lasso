...
export const ENDPOINT_CONFIG = Object.freeze({
  ...
  requestFormat: GENERATED_CONFIG.requestFormat === 'json' ? 'json' : 'multipart',
  ...
  successDialogTitle: String(GENERATED_CONFIG.successDialogTitle || 'Endpoint response'),
  pluginVersion: String(GENERATED_CONFIG.pluginVersion || '0.1.0'),
});

export function isNotePath(filePath) {
  return typeof filePath === 'string' && filePath.toLowerCase().endsWith('.note');
}
